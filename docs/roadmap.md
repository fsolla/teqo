...
- **B16+** escala/DRY pós-B16 — `useOptimistic` no Popover do header (hoje há um par `{baseKey,next}` manual com janela de reversão em cliques encadeados), payload dos rótulos de município (~20 KB por navegação) e hrefs do Popover a partir de uma base `URLSearchParams` (435 opções × 2 serializações hoje) · ~0,5–1d · sem migration · soft: antes de **B17**/**B18** reusarem a barra · [plano](plans/escala-dry-pos-b16.md)
