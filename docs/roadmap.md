...
- **B15** ordenar lista de municípios pelo header da coluna · `?sort=`/`?dir=` na URL + clique no header (desktop) / select compacto (mobile); ordenação global no filtrado · ~0,5–1d · Janela 1 · sem deps duras; **A11** consome o contrato (key `votos`) · cortável · [plano](plans/ordenacao-colunas-lista-municipios.md)
