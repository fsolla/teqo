...
- **A7 Escala e DRY pós-A4 — Fase 1 entregue (2026-07-19; merge pendente)** — agregação drizzle do ranking federal no detalhe (`loadFederalCandidateTotalsAggregated`); Fases 2–5 pendentes (`cityCode` TSE, Alert `confirmed` + helpers gap + `Progress`, fetch único 2022 lista+coroplético B3, batch federal flip/leverage na lista pós-A5). Passagens `/simplify` + `capture-review-debts` pós-B3, pós-A5 e **pós-A5-2 classificação territorial** (tally único, reuso tallies conversão, ordem classificação antes do flip N+1). [Plano](plans/escala-dry-pos-a4.md).
...
- **B5 Escala e DRY pós-B2 — F1 entregue com B3 (2026-07-19); Fases 2–3 pendentes** — lazy load de geometrias (split mun/TI + dynamic import) ✓; helper compartilhado de cache CLI (`ensureCachedDownload`) e factory mun/TI (`buildGeometryModuleFromTopology`) pendentes. `capture-review-debts` pós-B3 registrou F3. [Plano](plans/escala-dry-pos-b2.md).
- **B6 Escala e DRY pós-B3 registrado (2026-07-19)** — `BahiaMap` `setStyle` incremental (evitar rebuild GeoJSON a cada troca de métrica). `capture-review-debts` pós-2× `/simplify` B3. [Plano](plans/escala-dry-pos-b3.md).
...
| B6 Escala e DRY pós-B3                   | — (sem design novo; reusa mapa B3)                                               | [escala-dry-pos-b3.md](plans/escala-dry-pos-b3.md)                                                                                                                                                                                                                                                                                                                     |
...
flowchart TD
    ...
    subgraph TrilhaA["Trilha A — dados eleitorais e território"]
        ...
        A7["A7 Escala e DRY pós-A4<br/>(F1 agregar federal ✓ / cityCode / Alert / F4 fetch 2022)"]
    ...
    subgraph TrilhaB["Trilha B — superfícies de coordenação"]
        ...
        B5["B5 Escala e DRY pós-B2<br/>(F1 lazy ✓ / cache CLI / factory)"]
        B3["B3 Mapa Fase 2 (Leaflet nas superfícies)"]
        B6["B6 Escala e DRY pós-B3<br/>(setStyle Leaflet)"]
...
| 21    | A7 Escala e DRY pós-A4 (`cityCode` TSE, Alert `confirmed` + helpers gap + `Progress`, fetch único 2022 lista+coroplético, batch federal flip/leverage na lista) _(Fase 1 agregar federal entregue 2026-07-19; Fases 2–5 pendentes; F4 pós-B3; F5 pós-A5 via `capture-review-debts`)_ | [detalhes](plans/escala-dry-pos-a4.md)                                                                                                                                                                                                                                                | A4 ✓ (suave: A5/B3/E2 amplificam o custo)                                       | A5, A8, B3, B5, B6, C4, E4, D1 |
| 22    | B2 Mapa Fase 1 (geometrias) **(entregue 2026-07-18)** + B3 Leaflet nas superfícies                                                                                                                                                                                                   | [detalhes](plans/mapa-bahia-geometrias.md)                                                                                                                                                                                                                                            | B3 ← B1 ✓ + B2 ✓ (suave: A4 coroplético; E1/E2 métricas; B5 F1 lazy geometrias) | A5, A7, A8, B5, B6, C4, E4, D1 |
| 23    | B5 Escala e DRY pós-B2 (cache CLI + factory mun/TI; F1 lazy ✓)                                                                                                                                                                                                                       | [detalhes](plans/escala-dry-pos-b2.md)                                                                                                                                                                                                                                                | B2 ✓ (F1 entregue com B3)                                                       | A5, A7, A8, B3, B6, C4, E4, D1 |
| 24    | B6 Escala e DRY pós-B3 (`BahiaMap` setStyle incremental) _(cortável se troca de métrica for rara)_                                                                                                                                                                                   | [detalhes](plans/escala-dry-pos-b3.md)                                                                                                                                                                                                                                                | B3 (suave: A7 F4 na lista)                                                      | A5, A7, A8, B5, C4, E4, D1     |
| 25    | C4 Demandas                                                                                                                                                                                                                                                                          | [detalhes](plans/demandas-campanha.md)                                                                                                                                                                                                                                                | A1 (suave: C3 para a relação `actionPlan`)                                      | A5, A7, A8, B3, B5, B6, E4, D1 |
| 26    | D1 PWA `/campanha` **(entregue antecipado em 2026-07-18)**                                                                                                                                                                                                                           | [detalhes](plans/pwa-campanha.md)                                                                                                                                                                                                                                                     | —                                                                               | tudo                           |
| 27    | D2 Notificações (push + sino) — sino não depende do PWA; push sim                                                                                                                                                                                                                    | [detalhes](plans/notifications.md)                                                                                                                                                                                                                                                    | D1 ✓ (push) + chave `campanha-notificacoes-push` do lote jurídico               | A5, A7, A8, B3, B5, B6, C4, E4 |

**Janela 4 — set → 04/10 (reta final): dobradinha, dia D e estabilização.**

| Ordem | Item                                                                                                                                                                                                                                                                                                                                                                                                                          | Plano                                                             | Depende de                                                                                     |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 28    | A6 Insight dobradinha 2026                                                                                                                                                                                                                                                                                                                                                                                                    | [detalhes](plans/insight-dobradinha-2026.md)                      | A4 + TSE publicar candidaturas 2026 (externo, após 15/08) + taxonomia de alinhamento (produto) |
| 29    | B4 Camada de zonas TSE no mapa                                                                                                                                                                                                                                                                                                                                                                                                | ciclo seguinte do [plano do mapa](plans/mapa-bahia-geometrias.md) | A2 + B3                                                                                        |
| 30    | C5 **Operação dia D / GOTV** _(item proposto nesta revisão — validar com produto)_: confirmação de comparecimento da base nominal em 04/10 (lista de apoiadores por núcleo/zona com marcação "confirmou que vai votar / votou", visão de cobertura para a coordenação). É o uso final da base construída em C2; a literatura de campanha trata a mobilização do dia como onde eleições apertadas se decidem. Sem plano ainda. | design: [`Dia-D-GOTV`](design-refs/latest/Dia-D-GOTV.png)         | C2 (suave: C6 para escala da base)                                                             |
| 31    | Congelamento: a partir de ~20/09, só correção de bug e dados; nada de migration arriscada perto do dia D. E5 (Salvador por bairro) fica registrado para depois da eleição, condicionado a decisão de produto.                                                                                                                                                                                                                 | —                                                                 | —                                                                                              |
...
- **RS+ Escala e DRY pós-reset senha/perfil** (auth read `depth: 0`, avatar fora da TX, **auth UI parcial ✓** — shell/card/logo + critique login fechados; pendem `CampaignPasswordFields` + login em `CampaignFormActionState`) — engenharia pós-MVP; paralelizável com qualquer janela. → [detalhes](plans/escala-dry-pos-reset-senha-perfil.md)
