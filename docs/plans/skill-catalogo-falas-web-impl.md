...
- **Fix cross-item (S40) para destravar o gate.** O commit do plano S40 (`2515e86b`, main) sobrescreveu o asset aprovado do modelo do estadual (`docs/plans/cards-estadual-dobradinha-ui-design-assets/modelo-time-de-voce-com-estadual.jpeg`, sha `1f5dc821`) sem copiar os bytes para o caminho público `public/cards/modelo-time-de-voce-com-estadual.jpeg` (ainda `7fc8a50a`), deixando `tests/unit/stateDeputyCatalog.unit.spec.ts` vermelho no main. Este PR copia os bytes aprovados para o arquivo público — exatamente a decisão de gate já registrada no plano do S40, byte-a-byte, sem edição; o restante do S40 (remoção da silhueta desenhada, pins de `cardRender`/e2e) segue com a Issue #1316. Não é UI nova: é o port mecânico de uma arte já aprovada (non-trigger de design), necessário para o gate "Dono do PR, dono do CI".
