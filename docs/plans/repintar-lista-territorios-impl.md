...
- **`TerritoryListColumns.tsx`**: nova ordem visual fixa e classes responsivas por coluna — cada coluna não-core usa `hidden` combinado com o rung de container query (`@container/territory-list`), virando `table-cell` a partir de um breakpoint próprio em `rem`:
