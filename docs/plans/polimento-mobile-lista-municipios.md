...
- ~~**Slot `footer` + `formId` na casca, para o Sinal (B26) migrar.**~~ **Gatilho disparado em 2026-07-27**, na triage de fechamento do **E14 ✓**: `MunicipalityListLevelControl` é o 2º controle de submit explícito e migrou para a casca sem o slot, o que na folha pina "Fechar" e empurra "Registrar movimento" para baixo da dobra — a ação que descarta uma justificativa digitada ficou mais alcançável que a que a registra. Virou **F5** do **B32+** ([escala-dry-pos-b32.md](escala-dry-pos-b32.md)), junto do F4 desta mesma casca, e não um ID novo.
...
- **A ficha do município mantém Popover em qualquer viewport.** `MunicipalityGoalAccountCard` passa sempre `variant="popover"`, então no celular aquela edição rápida ainda briga com o teclado — o problema que este item resolveu uma rota ao lado. Não é esquecimento: a ficha é **uma** árvore (não o par `md:hidden`/`hidden md:block` das listas), então escolher o container ali exige `useIsMobile`, que este plano rejeitou por trocar o container depois da hidratação. Gatilho: queixa de campo sobre editar votos na ficha pelo celular — e aí a decisão a tomar é `useIsMobile` na ficha, não copiar o branch.
- **`DrawerBody` no kit.** `Drawer.tsx` exporta header/footer/title/description mas nenhum corpo, então há **dois** containers de scroll escritos à mão (a casca e o Sinal) que já divergem em `gap-3`/`pt-3`. Adicionar `DrawerBody` resolve, mas põe uma peça não-upstream no kit (`shadcn add` não a recoloca). Gatilho: o 3º corpo à mão.
- **Stub de `ResizeObserver`/`scrollIntoView` compartilhado.** Duas cópias (este spec e `campaignCompositionCleanup.unit.spec.ts`), abaixo da barra de 3 do repo; `vitest.unit.config.mts` não declara `setupFiles`, então não existe casa para mover hoje. Gatilho: o 3º spec jsdom que precise deles → `setupFiles: ['./tests/helpers/jsdomStubs.ts']`, não uma terceira cópia.
...
**Medido e não feito** (o revisor de performance mediu; a decisão é não pagar agora): cada Drawer fechado ainda executa o corpo de `DrawerContent`, e o `cn()` do popup roda `tailwind-merge` sobre **4.068 caracteres** — 6,2 µs contra 0,8 µs do Popover que ele substituiu, ou **+0,40 ms** em 25 linhas e **+7 ms** no caminho `pagination: false` de 435. Não é adição, é troca (o número de overlays por página não mudou), e a correção é montar o Drawer no primeiro `open` (`{everOpened && …}`, não `{open && …}`, senão o `data-starting-style` perde a animação de entrada). Registrado como débito com gatilho. **Bundle: zero delta nas duas rotas** — a lista já importava `ui/Drawer` pelo Sinal, e a ficha do município (que ganhou uma aresta nova via `MunicipalityGoalAccountCard`) já o tinha no grafo porque `InstallPwaToast`, no layout de `(app)`, o importa. Vale saber: se o `InstallPwaToast` sair do layout, a rota de detalhe passa a pagar a subárvore do Drawer só por essa aresta.
