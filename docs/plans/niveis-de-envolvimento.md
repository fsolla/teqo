# E14 — Níveis de envolvimento N0–N4 por Praça
...
Impeccable: B — campo novo no detalhe/lista da Praça (Popover B9) + badge staff; sem rota nova
Appetite: ~1 dia eng; migration pequena (campos em `plaza`)
Responsável: —

## Design (Impeccable)

Âncoras: `PRODUCT.md` (princípios 3 e 5) / `DESIGN.md` (register `product`) · controles B9 (`PlazaList*Control`).

Na implementação: craft compacto → critique → polish.

- **Persona / contexto:** coordenador na reunião quinzenal de realocação; assessor entendendo o que a praça dele "recebe".
...
Relatório §6.8: a escala N0 Monitorar → N1 Presença de mandato → N2 Rede sem agenda → N3 Rede+agenda → N4 Investimento pleno, com regras de movimento (promoção: gatilho positivo 2 semanas + capacidade; rebaixamento: não-resposta 3–4 semanas / perda estrutural / rebalanceamento; histerese: sem 2 mudanças no mês, sem pulo de 2 níveis salvo choque triangulado, janela de proteção de 3 semanas pós-promoção) e o rito (registro ex-ante com motivo E sinais de reversão; vocabulário duplo). Hoje `plaza.priority` só tem `alta|normal`. O nível modula a fila (E9), o motor (E11) e as metas (praça N0/N1 carrega meta mínima — K-B/FU3).

## Objetivos

- `plaza.engagementLevel` (`n0…n4`, default `n2`), `levelNote` (motivo corrente), `levelChangedAt` (derivado) — todos staff-only (access nega `leader` na leitura, mesmo mecanismo de `supportStatus`).
- Mudança de nível SEMPRE grava `allocationDecision` (patternId `nivel`, snapshot com nível anterior/novo, motivo, sinais de reversão) — histórico auditável sem versionar `plaza`.
...
- **Histórico via `allocationDecision`, não versions em `plaza`** (um mecanismo só para decisões — C12). **Rejeitado:** versions em plaza (custo alto, ruído); array `levelHistory` no doc (cresce sem access próprio).
- **Rebaixar meta ≠ rebaixar praça** (K-C): nível e meta são eixos independentes; a UI não acopla os dois automaticamente. **Rejeitado:** auto-derivar nível da cobertura (vira gaming imediato — G4).
...
- **Quem pode mudar nível?** Opções: só `coordinator` | `advisor` nas suas praças com aprovação. **Recomendação:** só `coordinator` (é decisão de realocação — rito §6.8); assessor propõe via nota/sinal.
...
flowchart LR
    ...
    Action["setPlazaEngagementLevel<br/>(valida movimento)"]
    Plaza["plaza.engagementLevel"]
...
- **`src/collections/Plaza.ts`**: 3 campos novos com access staff (padrão `politicalTrend`); `priority` sai da UI.
- **`src/utilities/plazaStaffFormActions.ts`** (ou action irmã): `setPlazaEngagementLevel` transacional (plaza + allocationDecision com `req`), validações de histerese puras em `src/lib/engagementLevel.ts` (unit-testável).
- **UI:** `PlazaListLevelControl` no padrão dos `PlazaList*Control` (B9); badge no header do detalhe.
- **Migration:** `pnpm migrate:create add_plaza_engagement_level`.

## Dependências

- Duras: **C12** (`allocationDecision` existir). Suaves: E8 (metas por nível), E9 (ordenação), E11 (nível no snapshot dos gatilhos).
- Reusa: `plazaStaffFormActions.ts`, `campaignAccess.ts`, padrão de field access de `supportStatus` (`Leadership.ts`).
...
- **Notificação de mudança de nível ao assessor da praça.** Gatilho: D2 (sino) entregue.
...
- `src/collections/Plaza.ts` (priority/politicalTrend/access), `src/collections/Leadership.ts` (padrão supportStatus)
- `src/utilities/plazaStaffFormActions.ts`, `src/components/campaign/PlazaListTrendControl.tsx` (padrão de controle B9)
