...
Impeccable: B — sem rota nova; campos novos em forms existentes (`plazaUpdate`, `actionPlan`) + collection admin-hidden
...
- **Edit where you see:** sim — o log de 1 linha nasce do mesmo form de atualização já usado no detalhe da Praça.
- **Anti-goals:** formulário longo obrigatório; sobrescrita silenciosa de valores; segundo feed paralelo ao `plazaUpdate`.

## Contexto

FU4 do relatório: backtest compara o que se sabia antes com o que aconteceu depois — "se o valor novo apaga o velho, o 'antes' deixa de existir". Hoje: `votePledge` guarda só o valor corrente (declarado/estimado sobrescrevem); `plazaUpdate` é imutável mas sem semântica de sinal (kinds `semanal|urgente|nota`); `actionPlan` não registra origem da visita (pedida × justificada — J-B) nem custo; e não existe registro de decisão (sugestão dada, aceita/descartada, leitura alternativa). Este item fecha os gaps G1–G4 do plano-mestre e é pré-requisito do motor (E11) e do backtest (E15). "O TSE estará lá em novembro; o lado da campanha não é reconstruível retroativamente."

## Objetivos

- **G1 — trajetória de pledge:** `versions: true` (sem drafts) em `votePledge`; leitura da série por praça/liderança para o delta semanal (E8) e o backtest (E15).
- **G2 — sinais tipados:** `plazaUpdate.kind` ganha `sinal`; campos condicionais `signalType` (`invasao | esfriamento | visita_adversario | proposta_broker | outro`), `signalSource` (texto curto), `triangulated` (checkbox staff-only). Continua imutável e mantendo `plaza.lastUpdateAt`.
...
- **G4 — decisões ex-ante:** collection `allocationDecision` (admin group `Campanha`, `admin.hidden` como `supporterImportBatch`): `plaza`, `patternId` (texto — P1…K-C ou `manual`), `suggestion` (texto), `decision` (`aceita | descartada | adiada`), `alternativeReading` (obrigatório ao descartar), `snapshot` (json — classificação/números no momento), `decidedBy` (derivado), `note`. Imutável após criação.
...
- **Decisão registrada é collection própria e imutável** — é o dado que o backtest paga caro para ter; mudanças de nível N0–N4 (E14) também gravam aqui, um mecanismo só. **Rejeitado:** log em `plazaUpdate` (semântica de feed de campo, autoria de liderança possível — decisão é de coordenação); campo no `plaza` (sobrescreve).
...
- **`versions` também em `plaza` (para foto da classificação)?** Opções: sim | não — snapshot vai no `allocationDecision.snapshot`. **Recomendação:** não; o snapshot por decisão cobre o "ex-ante" com custo menor que versionar 436 praças a cada edição.
...
flowchart LR
    ...
    Update["plazaUpdate kind=sinal<br/>(signalType/source/triangulated)"]
...
- **`src/collections/PlazaUpdate.ts`**: opção `sinal` no kind + 3 campos condicionais (`admin.condition` como os campos de `semanal`); access staff no `triangulated`.
...
- **`src/utilities/pledgeHistory.ts`**: leitura da série de versions por praça (consumida por E8/E15).
...
- Dura: deploy da remodelagem. Nenhuma de outro plano do programa (é a fundação; E8 usa se existir).
- Reusa: `withPayloadTransaction` (`payloadTransaction.ts`), `campaignAccess.ts`, padrão `createdBy` de `Supporter.ts`, `plazaUpdatePageData.ts` (feed).
...
- **"Já que estamos", versionar tudo.** Versions em `plaza`/`leadership` multiplica tabelas e ruído de admin. **Mitigação:** só `votePledge` neste item; demais têm gatilho abaixo.
...
- **`snapshot` virar dump gigante.** Limitar ao objeto pequeno (classe, cobertura, nível, números-chave) — não serializar o bundle da praça.

## Adiado com gatilho

- **Versions em `plaza` (histórico de estratégia).** Gatilho: E15 precisar de foto além do `allocationDecision.snapshot` em caso real.
...
- `src/collections/VotePledge.ts`, `src/collections/PlazaUpdate.ts`, `src/collections/ActionPlan.ts`, `src/collections/Supporter.ts` (padrão createdBy/access), `src/collections/SupporterImportBatch.ts` (padrão admin-hidden)
