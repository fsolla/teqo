...
- **Nome obrigatório e único na criação.** O nome é obrigatório (Salvar inativo sem nome) e **deve ser único entre as fichas** — comparação normalizada (sem espaços duplicados, sem caixa). Conflito na criação → **aviso claro ao usuário** ("Já existe um contato com este nome"), mantendo tudo o que foi digitado na linha nova / sheet.
...
- Criação exige nome (Salvar inativo sem nome); nome único (comparação normalizada); conflito → mensagem amigável mantendo o que foi digitado.
...
- **"Só completar" com unicidade de nome aplicada ao import CSV.** A base real já pode ter duas "Maria Silva" com telefones diferentes (import dedupa por telefone); bloquear nome no import quebraria o lote. **Corte:** a regra vale só para a criação/edição manual na página; sem dedupe retroativo.
