...
- **S6 — CSS compartilhado lista zona** (`MunicipalityMapPanel` zone breakdown ↔ `MunicipalityZoneNeighborhoodsCard`). **Obsoleto 2026-07-26:** o fill-in [remover-lista-zonas-mapa-inicio.md](remover-lista-zonas-mapa-inicio.md) remove a lista do mapa; sem segundo call site, não há o que extrair. O card de bairros no detalhe permanece.
- ~~**F2-D1 — card "Onde estou" (B14 ✓) resolver a ZE exata de Salvador.**~~ **Resolvido 2026-07-29** — `resolveNearbyMunicipality` + `NearestMunicipalityCard` usam a malha das 19 ZE; fallback: ZE acessível mais próxima (30 km) ou lista filtrada. Plano: [municipio-mais-proximo.md](municipio-mais-proximo.md).
