...
- **Lista de municípios:** ordenação nova `?sort=votos` (ano default 2022) em `municipalityUi.ts`/`campaignListUrl.ts`, preservando filtros/paginação; coluna/linha secundária com votos 2022 + % da própria votação.
...
- **i18n e naming:** `municipalityVoteRank.ts`, `computeVoteRankByYear`, `voteShareOfCandidate`, `?sort=votos`; labels pt-BR ("Posição em votos", "% da votação").
...
flowchart LR
    ...
    List["MunicipalityList + sort=votos<br/>(municipalityUi.ts)"]
    Artifact --> Rank --> Card
    Rank --> List
```

Componentes tocados: `src/utilities/municipalityVoteRank.ts` (novo, puro + unit tests), `municipalityUi.ts` (`MunicipalityListState.sort`), `MunicipalityFilters.tsx` (select "Ordenar por" — padrão auto-aplicado dos filtros), `MunicipalityList.tsx` (linha com votos/% quando ordenado por votos), `MunicipalityBaselineCard.tsx` (posição/share por ano), `municipalityPageData.ts` (ordenar o conjunto já carregado — o bundle A9+ carrega o conjunto filtrado inteiro; sem paginação SQL nova).

## Dependências

- Nenhuma dura (artefato em produção desde o hardening 2026-07-23). Relação: **E9** absorve `sort`/coluna como parte da fila (o select de ordenação nasce aqui e E9 adiciona `deficit`/`risco-frescor`); **E17** reusa o helper para o rollup por TI; **E4R** deixa `priority` real para o cruzamento visual.
...
- `docs/roadmap.md` (A11) · [fila-de-alocacao.md](fila-de-alocacao.md) (E9 — absorve) · [tabela-ti-inicio.md](tabela-ti-inicio.md) (E17 — reusa helper)
