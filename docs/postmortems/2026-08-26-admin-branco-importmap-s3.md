...
| Ambiente            | prod                                                |
| Issue(s)            | sem Issue                                           |
| PR do fix           | #926 (+ #927 e #929 para os desbloqueios do deploy) |
...
| Correção mergeada  | 2026-08-27 | PRs #926 (importMap +2 linhas + guard/pin), #927 (package.json revertido — desbloqueio 1 do deploy), #929 (override sharp — desbloqueio 2 do deploy)                             |
| Deploy             | 2026-08-27 | dispatch manual do deploy.yml (run 33074305879, 12:56 UTC) — verify 19m + deploy 16m verdes, revision `4d6533c0`                                                                 |
| Verificado em prod | 2026-08-27 | verificação read-only via browser: `/admin/login` renderiza o formulário (Email/Senha/Login, título "Login - Payload"), 0 erros de console; confirmação final do humano pendente |
...
- Prod: deploy do run 33074305879 (revision `4d6533c0`) concluído verde; verificação read-only via browser (2026-08-27): `/admin/login` renderiza o formulário com 0 erros de console — confirmação final do humano pendente
...
| Hardening da camada `deps` do Dockerfile (retry do `pnpm install` / libvips via apk / proxy do download do sharp) — o egress do homeserver para objects.githubusercontent.com não conclui downloads | cara   | resolvida em grande parte pelo PR #929 (override sharp → binários via npm registry, alcançável); retry/proxy seguem documentados para deps futuras que usem github releases                                                                                                                                |
