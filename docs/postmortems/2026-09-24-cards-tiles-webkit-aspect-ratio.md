...
- CI: o primeiro run do `checks` no PR #1321 falhou por uma **quebra pré-existente da main** — o S40 (`2515e86b`) sobrescreveu a arte aprovada em `docs/plans/cards-estadual-dobradinha-ui-design-assets/modelo-time-de-voce-com-estadual.jpeg` e o pin do S30 (`tests/unit/stateDeputyCatalog.unit.spec.ts`) passou a divergir; os deploys de S40/S39 na main também falharam (auto-unblock: Issue #1319; S40: Issue #1316). Nada do fix dos tiles dependia disso: a main voltou ao verde com `275dd15f`/`02ca4664` (S40 entregou a arte genérica e o arquivo público) e este push re-dispara o `checks` no merge ref já corrigido.
