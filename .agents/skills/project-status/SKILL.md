...
3. Se o usuário pedir interpretação além da saída (ex.: "o que destrava mais coisa?"), responda a partir do grafo — sem inventar dados que o script não imprimiu; se faltar dado, leia a Issue via API do GitHub (`gh` CLI / `pnpm issue` + `scripts/lib/github-api.mjs`) somente-leitura.
