...
| 2   | Dirty é só ruído local / efêmero: untracked scratch, IDE/OS (`.DS_Store`, `.idea/`, `*.swp`), cache, arquivos fora de `src/` `tests/` `docs/` `scripts/` `.agents/skills/` `.github/` e sem relação com o tip da branch                 | **stash** |
