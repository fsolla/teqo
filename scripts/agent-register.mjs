...
if (!flags.id || !flags.title) die('Usage: pnpm agent:register -- --id <ID> --title <title> [--prio P2] [--depends A,B] [--kind feature] [--plan path] [--body text] [--labels extra,labels] [--blocked]')

const priority = /^P[0-3]$/.test(flags.prio ?? 'P2') ? (flags.prio ?? 'P2') : die(`Invalid --prio "${flags.prio}" (P0..P3).`)
const depends = (flags.depends ?? '').split(',').map((v) => v.trim()).filter(Boolean)

const existing = issuesById().get(flags.id)
if (existing) die(`An issue with id ${flags.id} already exists: #${existing.number} — ${existing.title}`)
...
const labels = [
  ...
  ...(flags.labels ?? '').split(',').map((v) => v.trim()).filter(Boolean),
]
