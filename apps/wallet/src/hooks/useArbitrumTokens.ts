...
const ALCHEMY_ARB_URL = `https://arb-mainnet.g.alchemy.com/v2/${ALCHEMY_API_KEY}`;

export interface ArbitrumTokenBalance {
...
// Fetch token balances from Alchemy Arbitrum
const fetchTokenBalances = async (
  address: string
): Promise<AlchemyTokenBalance[]> => {
  const response = await fetch(ALCHEMY_ARB_URL, {
    method: "POST",
...
// Fetch token metadata from Alchemy Arbitrum
...
// Fetch all Arbitrum token data (balances + metadata)
