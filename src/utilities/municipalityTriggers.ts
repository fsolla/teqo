...
import { computeGoalCoverage } from '@/utilities/goalCoverage'
import { loadStatewideSuggestedGoals } from '@/utilities/municipalityGoalAccount'
import {
...
import { countLeadershipsByMunicipality } from '@/utilities/visitPlannerData'
...
export const loadMunicipalitySuggestions = async (
  ...
  const municipalities = result.docs as TriggerMunicipalityDoc[]
  ...
  const [
    ...
    // The unfiltered read is the dashboard's, and the dashboard already paid
    // for the scope (React cache) — re-aggregating here would scan the whole
    // votePledge table a second time on the same request.
    municipalityID
      ? aggregatePledgesByMunicipality(payload, municipalityIDs)
      : loadMunicipalityScope(payload, user, {}).then((scope) => scope.pledgeAggregates),
