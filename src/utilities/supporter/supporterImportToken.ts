...
// Intentional admin bypass in every helper below: the staging collection is
// actor-bound and token-gated by design (HMAC verify + actorID + TTL checked by
// the caller before any of these run), so the collection's own access rule
// would only duplicate checks already made — the bypass is the module's policy.
