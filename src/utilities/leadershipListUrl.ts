...
export const buildLeadershipSortHref = (
  state: LeadershipListState,
  nextKey: LeadershipListSortKey,
): string => {
  const current = resolveLeadershipListSort(state)
  const dir =
    current.sort === nextKey
      ? current.dir === 'asc'
        ? 'desc'
        : 'asc'
      : defaultLeadershipListSortDir(nextKey)

  return buildLeadershipListHref({ ...state, sort: nextKey, dir, page: 1 }, 1)
}
