...
import type { TerritorySortDir, TerritorySortKey } from '@/utilities/territoryOverview'
...
const parseRegions = (value: string | string[] | undefined): BahiaIdentityTerritory[] => {
...
export const buildTerritorySortHref = (
  state: TerritoryListState,
  nextKey: TerritoryListSortKey,
): string => {
  const current = resolveTerritoryListSort(state)
  const dir =
    current.sort === nextKey
      ? current.dir === 'asc'
        ? 'desc'
        : 'asc'
      : defaultTerritoryListSortDir(nextKey)
  return buildTerritoryListHref({ ...state, sort: nextKey, dir })
}
