import { CAMPAIGN_CACHE_PREFIX, CAMPAIGN_PWA_CLEAR_CACHES_MESSAGE } from '@/utilities/campaignPwa'
...
export const clearCampaignPwaCaches = async (): Promise<void> => {
  try {
    ...
    await Promise.all(
      keys.filter((key) => key.startsWith(CAMPAIGN_CACHE_PREFIX)).map((key) => caches.delete(key)),
    )
  } catch {
