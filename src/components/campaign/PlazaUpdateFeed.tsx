...
import { plazaUpdateKindLabels } from '@/lib/schemas/plazaUpdate'
import type { PlazaUpdateViewModel } from '@/utilities/plazaUpdatePageData'
...
export const PlazaUpdateFeed = ({ updates }: { updates: PlazaUpdateViewModel[] }) => {
  if (!updates.length) {
    return (
      ...
      {updates.map((update) => (
        <li key={update.id} className="flex flex-col gap-3 rounded-xl border p-4">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={kindVariant[update.kind]}>{plazaUpdateKindLabels[update.kind]}</Badge>
