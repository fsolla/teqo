...
import {
  actionPlanDetailTabs,
  buildActionPlanDetailTabHref,
  type ActionPlanDetailSearchParams,
  type ActionPlanDetailTab,
} from '@/utilities/actionPlanDetailTabUi'

const tabLabels: Record<ActionPlanDetailTab, string> = {
  overview: 'Visão geral',
  tasks: 'Tarefas',
  updates: 'Atualizações',
}

export const ActionPlanTabNav = ({
  activeTab,
  planSlug,
  searchParams,
}: {
  ...
  >
    ...
    >
      {actionPlanDetailTabs.map((tab) => {
        const isActive = tab === activeTab
        return (
          <li key={tab} className="m-0 h-full p-0">
            <Link
              href={buildActionPlanDetailTabHref(planSlug, tab, searchParams)}
              scroll={false}
