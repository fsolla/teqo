...
const serializeTasks = (tasks: ActivityTaskFieldValue[]) =>
  JSON.stringify(
    tasks
...
export const ActivityTaskFields = ({
  initialTasks,
  searchContacts,
  error,
}: ActivityTaskFieldsProps) => {
  const fieldId = useId()
  const [tasks, setTasks] = useState<ActivityTaskFieldValue[]>(() =>
  ...
  const updateTask = (key: string, patch: Partial<ActivityTaskFieldValue>) => {
