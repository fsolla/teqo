...
import { usePlazaEstimateScenario } from '@/components/campaign/PlazaEstimateScenarioContext'
import {
...
export const PlazaListOverview = ({ view }: { view: PlazaListOverviewData }) => {
  const { scenario, setScenario } = usePlazaEstimateScenario()
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 border-b border-foreground/10 px-4 py-2 sm:flex-row sm:items-end sm:justify-between">
        <VoteEstimateScenarioField
          id="plaza-overview-estimate-scenario"
          value={scenario}
