...
export const MunicipalityZoneNeighborhoodsCard = ({
  municipalitySlug,
}: {
    ...
    >
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          {neighborhoods.length} {neighborhoods.length === 1 ? 'bairro' : 'bairros'} ·{' '}
          {municipalityZoneNeighborhoodSourceLabel(source)}
        </p>
      </div>
      <ul className="grid gap-1.5 sm:grid-cols-2">
        {neighborhoods.map((neighborhood) => (
          <li key={neighborhood} className="rounded-md border px-3 py-2 text-sm text-foreground">
            {neighborhood}
          </li>
        ))}
      </ul>
