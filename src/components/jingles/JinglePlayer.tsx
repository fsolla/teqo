...
/**
 * S21 — the jingles list (artefato: cenas 01/02). One client component owns the
 * exclusivity state: every card renders its own `<audio preload="none">` (no
 * request before the play, per the gate note) and starting one pauses the
 * others. The play control reverts on a failed `play()` so `aria-pressed` never
 * gets stuck; progress/time come from the media events.
 */
              ...
              <div className="p-4 md:p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <h2 className="m-0 truncate border-b-0 pb-0 text-left font-[family-name:var(--font-exo2)] text-2xl leading-none font-black tracking-[-0.02em] md:text-[26px]">
                      {jingle.title}
                    </h2>
                    {playing ? (
                  ...
                  <div className="mt-2 flex justify-between text-[10px] font-bold tracking-wide text-black/50 uppercase md:text-[11px]">
                    ...
                    <span>
                      ...
                      {state.durationSeconds
                        ? formatSpeechClock(state.durationSeconds)
                        : EMPTY_TIME}
                    </span>
                  </div>
                </div>
