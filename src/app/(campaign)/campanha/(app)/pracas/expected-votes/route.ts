...
import { plazaStaffEditSafeMessages } from '@/app/(campaign)/campanha/(app)/pracas/plazaStaffEditMessages'
import { setPlazaExpectedVotes } from '@/app/(campaign)/campanha/actions/plaza'
...
import type { PlazaListExpectedVotesResponse } from './types'

export type { PlazaListExpectedVotesResponse } from './types'

export const dynamic = 'force-dynamic'

const optionalEstimate = z.number().int().min(0).max(1_000_000).nullable()

/** Unordered draft body — setPlazaExpectedVotes normalizes before persist. */
const bodySchema = z.object({
  plazaId: positiveRelationshipId,
...
export async function POST(
  request: Request,
): Promise<NextResponse<PlazaListExpectedVotesResponse>> {
  ...
  try {
    const { plazaId, expectedVotes } = bodySchema.parse(json)
    const updated = await setPlazaExpectedVotes({ plaza: plazaId, expectedVotes })
