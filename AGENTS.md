...
## Known Gaps (as of 2026-07-19 — resolve before relying on this file for onboarding new devs)

Backlog consolidado (próximos passos, bloqueadores, site, campanha, white-label): [`docs/roadmap.md`](docs/roadmap.md). Status operacional de Núcleos: [`.cursor/rules/projects/nucleos-eleitorais.mdc`](.cursor/rules/projects/nucleos-eleitorais.mdc).
...
4. **Campaign LGPD — textos finais + PII real (Onda 0).** Provisórios MVP já provisionados por migration / `pnpm db:seed:consent` (`lideranca-autopreenchimento`, `apoiador-cadastro`, `apoiador-intencao-voto`, `campanha-notificacoes-push` + global `/privacidade`). O app ainda falha fechado se as chaves faltarem. **Hold operacional:** não cadastrar titulares reais nem importar base nominal em massa até a assessoria jurídica eleitoral aprovar e substituir os textos definitivos (lote único — ver [`docs/plans/onda-0.md`](docs/plans/onda-0.md) e checklist abaixo). C2 engenharia está em `main`; dados reais ↔ Onda 0.

**Já no produto (resumo — detalhe e próximos em [`docs/roadmap.md`](docs/roadmap.md)):** núcleos MVP + Ciclo 2 (A1–A4, B1–B2, C1–C3, D1); C2 eng. + C6–C9 escala apoiadores; Trilha E parcial (E1–E3); Onda 0 eng. (Consent provisório); fill-ins Visitados / Reset senha+perfil. Débitos abertos (A5–A8, B3–B5, C4–C5, C10–C11, D2, E4–E7, O0+/VR+/RS+/FD+/FD2) só no roadmap.
...
Nominal supporter registry backing the campaign's base of declared supporters (roadmap item C2; engineering ready and merged to `main` on 2026-07-18; real PII hold ↔ Onda 0 legal batch). One collection in admin group `Campanha`:
...
- **C6–C9 scale layers (merged to `main` 2026-07-19).** C6 bulk import + KPI + shells; C7 território/leituras; C8 locks/bulk/`pg_trgm`; C9 filtros unificados / loader. Open scale follow-ups: **C10** / **C11** (`docs/plans/escala-dry-pos-c9.md`, `docs/plans/escala-dry-pos-c7.md`).
- **Mandatory production blocker (PII real):** provisional `apoiador-cadastro` / `apoiador-intencao-voto` keys are already seeded (Onda 0). Do not import real supporter data or capture vote intention until electoral counsel documents the LGPD art. 11 basis (vote intention is sensitive data) and approves the **final** versioned consent texts that replace the provisórios. The app fails closed if keys are absent; with provisórios present, the hold is legal/operational.
...
6. **Mandatory production blocker (PII real):** provisional Consent keys + `/privacidade` are auto-provisioned (Onda 0 migrations / `pnpm db:seed:consent`). Do not insert real leadership/supporter PII or enable mass invites/imports until electoral counsel approves the **final** versioned texts (including `lideranca-autopreenchimento`). The app fails closed if keys are absent; with provisórios present, the hold is legal/operational — see [`docs/plans/onda-0.md`](docs/plans/onda-0.md).
